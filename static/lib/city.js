function showcity(province,cityField,default_option){
    switch (province) {
        case "北京" :
    ...
    cityField.children("option").remove();
    for(var i = 0; i < cityOptions.length; i++) {
        if(default_option.indexOf(cityOptions[i]) >= 0){
            //cityField.append(new Option(cityOptions[i],cityOptions[i],true));
            cityField[0].options.add(new Option(cityOptions[i],cityOptions[i],true));
        }else{
            //cityField.append(new Option(cityOptions[i],cityOptions[i],false));
            cityField[0].options.add(new Option(cityOptions[i],cityOptions[i],true));
        }
    }
}